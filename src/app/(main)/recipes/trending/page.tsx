...
function mapTrendingToCardRecipe(
  recipe: RecipeDto,
  ...
  const isFavorite =
    currentUserId != null &&
    recipe.favorites?.some(fav => String(fav.userId) === String(currentUserId));
...
export default function TrendingRecipesPage() {
  ...
  // Fetch trending recipes with TanStack Query
  // Note: Using a large page size to get all trending, then filter client-side
  const {
    data: queryData,
    isLoading,
    error,
  ...
  // Extract recipes from response - handle different response formats
  const dataContent = useMemo((): RecipeDto[] => {
    if (!queryData) return [];
    // If queryData is already an array of recipes
    if (Array.isArray(queryData)) {
      return queryData as RecipeDto[];
    }
    // Cast to check for different property names
    const responseData = queryData as {
      content?: RecipeDto[];
      recipes?: RecipeDto[];
    };
    // Check for 'recipes' property (trending API format)
    if (responseData.recipes && Array.isArray(responseData.recipes)) {
      return responseData.recipes;
    }
    // Check for 'content' property (standard paginated format)
  ...
  const allRecipes = useMemo(() => {
    ...
    dataContent.forEach((recipe: RecipeDto) => {
      if (recipe.tags) {
        recipeTagsMap.set(
          recipe.recipeId,
          recipe.tags.map((t: { name: string }) => t.name)
        );
      }
    });
    return dataContent.map((recipe: RecipeDto) =>
      mapTrendingToCardRecipe(recipe, currentUserId)
    );
  }, [dataContent, currentUserId]);
        ...
        <main className="flex-1">
          {hasNoRecipes ? (
            <EmptyState variant="default" size="lg">
              ...
              <EmptyStateTitle>No trending recipes right now</EmptyStateTitle>
              <EmptyStateDescription>
                Check back later for recipes that are gaining popularity!
              </EmptyStateDescription>
              <EmptyStateActions>
                <Button asChild>
                  <Link href="/recipes/popular">
