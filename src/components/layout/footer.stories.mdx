...
<Canvas>
  <Story name="Complete">{() => <Footer />}</Story>
</Canvas>

## Without Build Info

<Canvas>
  <Story name="No Build Info">{() => <Footer showBuildInfo={false} />}</Story>
</Canvas>
...
<Canvas>
  <Story name="Custom Style">
    {() => <Footer className="bg-muted border-primary border-2" />}
  </Story>
</Canvas>
...
<Canvas>
  <Story name="Interactive">{args => <Footer {...args} />}</Story>
</Canvas>
