  ...
  {() => {
    ...
    return (
      <div className="space-y-2">
        ...
        />
        <button
          className="bg-secondary ml-2 rounded-md px-3 py-1 text-sm"
...
<button onClick={() => setAttempt(1)}>Reset Attempts</button>`} language="tsx"
/>
    ...
    return (
      <div className="space-y-4">
        <RetryButton
          ...
          onAfterRetry={result =>
            addLog(
              `Attempt ${result.attemptNumber} ${result.success ? 'succeeded' : 'failed'}`
            )
          }
