...
```tsx
<Collapse>
  <CollapseTrigger showIcon={false}>Simple collapse</CollapseTrigger>
  <CollapseContent>
...
>
  ...
  <CollapseContent>
    <p>
      Can substitute with vegetable shortening, coconut oil, or vegan butter.
    </p>
  </CollapseContent>
</IngredientNotesCollapse>
```
...
```tsx
<CollapseGroup spacing="tight" allowMultiple={false} defaultOpenItems={[0]}>
  <Collapse variant="outlined">
...
<Collapse key={contentVersion}>
  ...
  <CollapseContent>{/* Content that might change size */}</CollapseContent>
</Collapse>
```
